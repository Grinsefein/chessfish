...
export default function EngineSettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
      {/* Header */}
      <header className="h-16 border-b-2 border-zinc-900 bg-zinc-950 flex items-center px-8 shrink-0">
        <button
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors mr-8"
        >
          <ArrowLeft size={18} />
          <span className="text-sm font-black uppercase tracking-wider">Back</span>
        </button>
        <div className="flex items-center gap-5">
          <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center shadow-[0_4px_0_0_#4a6728]">
            <Cpu className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-black text-white tracking-tight uppercase">Engine Settings</h1>
      ...
      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full">
        ...
        >
          {/* Sidebar Navigation */}
          <div className="w-64 border-r-2 border-zinc-800 bg-zinc-950 p-4 shrink-0 overflow-y-auto">
          ...
          {/* Content Area */}
          <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-8 bg-zinc-900">
            ...
            {/* Backend Tab */}
            <TabsContent value="backend" className="m-0 space-y-8">
              <div className={cn(
                "p-6 rounded-2xl border-2 flex items-center justify-between relative overflow-hidden",
                status === 'ready' ? 'bg-green-500/10 border-green-500/50' :
                status === 'booting' ? 'bg-amber-500/10 border-amber-500/50' :
